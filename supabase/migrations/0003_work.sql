...
create index tasks_today_idx   on public.tasks (user_id, scheduled_for) where status <> 'done';
create index tasks_due_idx     on public.tasks (user_id, due_at)        where status <> 'done';
create index tasks_inbox_idx   on public.tasks (user_id, created_at desc) where status = 'inbox';
...
create index habit_logs_recent_idx on public.habit_logs (habit_id, logged_on desc);
